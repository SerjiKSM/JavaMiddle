...
public class MyClass {
   public static void main(String[] args) {
	   ...
	   Car car = new BMW();  // здесь будут доступны все методы класса Car
	   System.out.println("Car weight: " + car.getWeight()); 
	   
	   Colored c = (Colored)car;  // здесь преобразовуем переменную car к типу интерфейса Colored и здесь будет доступен только один метод
	   System.out.println("Car color: " + c.getColor());
	   
	   Hamburger h = new Hamburger();  // будет вызвана ошибка потому что класс Hamburger не наследуется и не имплементируется от интерфейса Colored соответственно у него  
	   c = (Colored)h; // ClassCastException !!!                                                // нет доступа к методу getColor() интерфейса Colored
